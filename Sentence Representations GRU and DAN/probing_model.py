...
class ProbingClassifier(models.Model):
    ...
    def call(self, inputs: tf.Tensor, training: bool =False) -> tf.Tensor:
        ...
        """
        # TODO(students): start
        proc_input = self._pretrained_model(inputs)
        layer_reprez = proc_input['layer_representations']
        # self.linear_class.trainable = training
        curr_layer = layer_reprez[:, self._layer_num-1, :]
        output = self.linear_class(curr_layer)
        logits = output
        # TODO(students): end

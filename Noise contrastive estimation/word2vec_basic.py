...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  """
    ...
    while (window_end < len(data) and i < batch_size):
        subset = data[window_start:window_end]
        ## Get the mid word and repeat
        mid = len(subset) // 2
        contex_word = subset[mid]
        temp = [contex_word] * (window_size - 1)
        contex += temp
        ## Get the previous and the next words
        prev = subset[:mid]
        nxt = subset[mid + 1:]
    ...
    # print (contex, target)
    batch = np.array(contex[:batch_size])
    target = np.array(target[:batch_size]).reshape((len(target[:batch_size]), 1))
    return batch, target
...
def train(sess, model, data, dictionary, batch_size, num_skips, skip_window,
    ...
    for step in xrange(max_num_steps):
        ...
        if step % checkpoint_step == 0:
            ...
            for i in xrange(valid_examples.shape[0]):  # make it io valid size later ####
    ...
    print("here it is...")
    print(dictionary['american'])
    print(dictionary['first'])
    print(dictionary['would'])
...
if __name__ == '__main__':
    ...
    valid_examples = np.array([64, 46, 74])  # np.random.choice(valid_window, valid_size, replace=False)
